/* ═══════════════════════════════════════════════════════════════
   Eros v5 — Method: Edifice v3 (Tectonic Topology)
   Evolution of Edifice v2 with full taxonomy compliance.
   
   v3 Additions over v2:
   ─ Color Stability: position-deterministic color (immune to physics changes)
   ─ Exposed Physics: springK, damp, simSteps, forceMin/Max, bounceEnergy
   ─ Squish Displacement: parity-based trapezoidal shear from taxonomy
   ─ RK4 Flow Hatch: curved flow-field crosshatching per enclosure
   ─ Mass-Dependent Density: smaller enclosures = denser hatching
   ─ Topological Density Clamping: prevents ink black-hole collapse
   ─ Enclosure Phase: position-derived, deterministic per cell
   ═══════════════════════════════════════════════════════════════ */

MethodRegistry.register({
  id: 'edifice-v3',
  name: 'Edifice v3 (Tectonic)',
  category: 'Eros Heuristics',
  version: '6.0.0',
  description: 'Brutalist mereological tectonics with deterministic color, controllable physics, RK4 hatching, and topological density regulation.',
  ...
  params: [
    ...
    { key: 'displacement', type: 'select', label: 'Displacement Type',    default: 'None', options: ['None', 'Twist', 'Sharp', 'Shift', 'Squish'], category: 'Physics' },
  ...
  render(canvas, ctx, W, H, params, palette) {
    ...
    for (let enc of enclosures) {
      ...
      // ── RK4 Flow hatching ──
      if (doRK4Flow) {
        ctx.lineWidth = params.lineWeight * (doSpringMesh ? 0.6 : 1.0);
  ...
  _displacePoint(px, py, cx, cy, type) {
    ...
    if (type === 'Twist') {
      ...
      // v3 NEW — Parity-based trapezoidal shear from taxonomy
      // Alternating compression/expansion bands
      const rowParity = Math.floor(py * 0.04) % 2;
      const squishScale = (rowParity === 0) ? 1.4 : 0.6;
      x = cx + dx * squishScale;
      // y remains unchanged — horizontal shear only
    }
